  ...
  >
    ...
    >
      ...
      <el-table-column
        align="center"
        prop="account_name"
        :label="$t('account-name-and-number')"
      >
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      tableData: state => {
        if (state.Accounting.Reports.auxiliaryReport.records?.length) {
          return state.Accounting.Reports.auxiliaryReport.records;
        } else {
          return [];
        }
      },
      tableInfo: function(state) {
        return state.Accounting.Reports.auxiliaryReport.recordsInfo.map(
          item => {
