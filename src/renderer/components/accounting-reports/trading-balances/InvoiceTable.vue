...
export default {
  ...
  computed: {
    ...
    ...mapState({
      tableData: state => {
        if (state.Accounting.Reports.tradingBalances.records?.length) {
          return state.Accounting.Reports.tradingBalances.records;
        } else {
          return [];
        }
      },
      tableInfo: function(state) {
        return state.Accounting.Reports.tradingBalances.recordsInfo.map(
          item => {
            let totalLabel = item.accName
              .replace(/#/g, "")
              .trim()
              .split(" ")
              .join("-")
