...
export default {
  ...
  computed: {
    reportData() {
      let filter = this.$store.state.Accounting.Reports.tradingBalances
        .RecordDetails;
      let branchName = null;
      ...
      return {
        reportPath: reportsPaths["trading-balances"],
        headerPath: reportsPaths["headerCompany"],
        dataSet: `uri=${
          this.$config.axios.baseURL
        }accounting/reports/trading-balances;jpath=$;Header$Authorization=bearer${" " +
          localStorage.getItem("accessToken")};Header$Accept-Language=ar-SA`,
        connString:
          this.data != null
            ? "jsondata=" + JSON.stringify(this.data.data)
            : JSON.stringify(
                this.$store.state.Accounting.Reports.tradingBalances.records
              ),
  ...
  methods: {
    async displayRecord() {
      let response = this.$store
        .dispatch("Accounting/Reports/tradingBalances/fetchRecords")
        .catch(err => {
