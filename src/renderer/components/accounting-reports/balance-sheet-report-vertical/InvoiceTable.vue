<template>
  ...
  >
    ...
    >
      ...
      >
        <template slot-scope="scope">
          {{ $numberWithCommas(scope.row.debit1) }}</template
        >
      </el-table-column>
      <el-table-column
        align="center"
      ...
      >
        <template slot-scope="scope">
          {{ $numberWithCommas(scope.row.credit1) }}</template
        >
      </el-table-column>
    </el-table>
    ...
    >
      ...
      <el-table-column align="center" prop="debit_balance">
        <template slot-scope="scope">
          {{ $numberWithCommas(scope.row.debit_balance) }}</template
        >
      </el-table-column>
      <el-table-column align="center" prop="credit_balance">
        <template slot-scope="scope">
          {{ $numberWithCommas(scope.row.credit_balance) }}</template
        >
      </el-table-column>
    </el-table>
  </el-container>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      tableData: state => {
        if (
          state.Accounting.Reports.balanceSheetReportVertical.records?.length
        ) {
          return state.Accounting.Reports.balanceSheetReportVertical.records;
        } else {
          return [];
        }
      },
      tableInfo: function(state) {
        return state.Accounting.Reports.balanceSheetReportVertical.recordsInfo.map(
          item => {
            let totalLabel = item.accName.replace(/#/g, "").trim();
            return {
              total: this.$t(totalLabel),
