...
export default {
  ...
  computed: {
    reportData(){
      let filter =this.$store.state.Accounting.Reports.balanceSheetReportVertical.RecordDetails;
      let branchName = null;
      ...
      if(filter.branchID){
        ...
        let branch = branchesList.find(item => item.id === filter.branchID)
        branchName = branch.name
      }
      return {
        reportPath: reportsPaths["balance-sheet-report-vertical"],
        headerPath: reportsPaths["headerCompany"],
        dataSet: `uri=${this.$config.axios.baseURL}accounting/reports/balance-sheet-report-vertical;jpath=$;Header$Authorization=bearer${" "+localStorage.getItem("accessToken")}`,
        connString:this.data!= null ? "jsondata=" + JSON.stringify(this.data.data):JSON.stringify(this.$store.state.Accounting.Reports.balanceSheetReportVertical.records),
  ...
  methods: {
    async displayRecord() {
      let response = this.$store
        .dispatch("Accounting/Reports/balanceSheetReportVertical/fetchRecords")
        .catch(err => {
