<template>
    <el-dialog class="pt-0-dialog" title=" " :visible.sync="showDialog" width="35%">
      ...
      <el-row>
        <div class="title width-full">{{$t('please-select-delivery-delegate')}}</div>
      </el-row>
      ...
      <el-row style="padding: 0 2rem;">
        <div class="select-container box-shadow mt-2">
          ...
          <div class="names-list">
            <el-menu v-model="delivery_delegate">
...
export default {
  ...
  methods: {
    closeDialog() {
      this.$store.commit("pos/delegateSelect/updateDialogState", false);
    },

    select_delegate(index) {
      this.delivery_delegate = index;
    }
  },
  ...
  computed: {
    showDialog: {
      set(state) {
        return this.$store.commit("pos/delegateSelect/updateDialogState", state);
      },

      get() {
        return this.$store.state.pos.delegateSelect.showDialog;
      },
    },

    search_target: {
      set(state) {
        return this.$store.commit("pos/delegateSelect/updateSearchTarget", state);
      },

      get() {
        return this.$store.state.pos.delegateSelect.updateSearchTarget;
      },
    }
  },
};
