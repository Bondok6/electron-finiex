  ...
  watch:{
    singleRecordDetails: {
      handler(newVal){
        ...
        this.itemType = false;
        for(let i=0 ; i<newVal?.detailsItemsReceiptBranches?.length ;i++){
          if(newVal.detailsItemsReceiptBranches[i].typeStock === 1) {
            this.itemsAttributes = true;
          }
          if(newVal.detailsItemsReceiptBranches[i].typeStock === 2) {
            this.patchAndExpiredDate = true;
          }
          if(newVal.detailsItemsReceiptBranches[i].itemType === 3) {
            this.itemType = true;
          }
        };

        this.data = newVal?.detailsItemsReceiptBranches?.map(item => {
          return {
            ...item,
            showDialog: false,
