          ...
          <el-row :gutter="6">
            <el-col :xs="24" :sm="6" :md="5" :lg="4">
              <el-form-item :label="$t('transfer-number')">
                <el-input v-model="form.invoiceId" @blur="getRecordDetails"/>
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="6" :md="5" :lg="4">
              <el-form-item :label="$t('branch-transferred-from')">
                <el-select
                      class="width-full"
                      v-model.number="form.fromBrancheId"
...
export default {
  ...
  async created(){
    await Promise.all([
    this.$store.dispatch("inventory/receiptsBetweenBranches/fetchMaxCode").then(res =>{
      this.form.invoiceCode = res.data.data;
    }).catch(err => {
      console.log(err);
    }),
    this.$store.dispatch("inventory/receiptsBetweenBranches/fetchCurrentBranch").then(res => {
      this.form.toBrancheId = res.data.data.branceId;
      this.form.toBrancheName = res.data.data.branceName;
    }).catch(err => {
      console.log(err);
    }),
    ])
  ...
  methods: {
    ...
    getRecordDetails() {
      if(this.form.invoiceId){
        this.$store.dispatch("inventory/receiptsBetweenBranches/fetchSingleRecordDetails", 
        { invoiceId : this.form.invoiceId,
          WarehouseId: this.form.fromWarehouseId,
          BrancheId: this.form.fromBrancheId}).then(re=>{
            this.$message(re);
          });
      }
    },
  ...
  watch: {
    singleRecordDetails: {
      handler({ detailsItemsToReceipt, toBrancheId, toBrancheName,...data}) {
        this.form = {
          ...data,
          toBrancheId: this.form.toBrancheId,
          toBrancheName: this.form.toBrancheName,
