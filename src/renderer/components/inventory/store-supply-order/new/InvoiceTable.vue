...
export default {
  ...
  computed :{ 
    ...
    ...mapState({
      ...
      recordDetails: state => state.inventory.storeSupplyOrder.recordDetails,
    })
  },
  ...
  methods: {
    ...
    getItemDetalis(scope){
      this.$store.dispatch("inventory/storeSupplyOrder/fetchItemDetails",
        scope.row.itemID
      ).then(res => {
    ...
    checkQuantity(scope) {
      ...
      > comparedQuantity && this.authenticated) {
      this.$confirm(
