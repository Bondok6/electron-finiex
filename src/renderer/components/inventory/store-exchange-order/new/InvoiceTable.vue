...
export default {
  ...
  methods: {
    ...mapMutations({
      setRecordDetails: "inventory/storeExchangeOrder/setRecordDetails"
    }),
    remoteMethod(query) {
      this.loading = true;
      this.$store
    ...
    getItemDetalis(scope) {
      this.$store
        .dispatch(
          "inventory/storeExchangeOrder/fetchItemDetails",
          scope.row.itemID
        )
        .then(res => {
          ...
          if (!scope.row.warehouseID) {
            scope.row.warehouseID =
              res.data.data.quantityWarehouse[0].warehouseID;
            scope.row.actualQuantity =
              res.data.data.quantityWarehouse[0].actualQuantity;
    ...
    openDialog(scope) {
      ...
      this.$nextTick(() => this.$refs["dialogDetails" + scope.$index].focus());
    },
    closeDialog(scope) {
    ...
    getQuantityAv(scope) {
      for (let item of scope.row.quantityBatches) {
        if (
          item.expireDate === scope.row.expireDate ||
    ...
    checkSerialNum(scope) {
      if (
        scope.row.quantity &&
        scope.row.serialNumber.length < scope.row.quantity
      ) {
    ...
    checkQuantity(scope) {
      ...
      if (
        scope.row.quantity * scope.row.quantityFull > comparedQuantity &&
        this.authenticated
      ) {
        this.$confirm(
          this.$t("transfer-between-branches-quantity-warning") +
            ` (${scope.row.quantity}) ` +
            this.$t("ask-for-continue"),
          {
      ...
      if (scope.row.quantity * scope.row.quantityFull < 0) {
        this.$confirm("لا يمكن ادخال كمية اقل من الصفر !", {
          confirmButtonText: this.$t("ok"),
