<template>
  <el-row class="mt-2 mx-3">
    ...
    <NuxtLink :to="localePath('/inventory/transfer-between-branches')">
...
<script>
  ...
  export default {
    computed: {
      ...mapState({
        total: state => state.inventory.transferBetweenBranches.recordDetails.total,
      }),
    },
    methods: {
      update() {
        this.$store.dispatch("inventory/transferBetweenBranches/update"
        ).then(()=>{
          this.$notify({
            title: "success",
            type: "success",
          ...
          this.$router.push("/inventory/transfer-between-branches");
        }).catch(()=>{
          this.$notify({
            title: "Error",
            type: "error",
      ...
      this.$confirm(this.$t("message-when-delete-record"), "Warning", {
        ...
        .then(() => {
          return this.$store.dispatch(
            "inventory/transferBetweenBranches/delete",
