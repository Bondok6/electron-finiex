<template>
  <div class="justify-center mt-2 action-buttons-nonGrown align-center align-baseline">
    ...
    <NuxtLink :to="localePath('/accounting/receipt-compound-vouchers')">
    ...
    create() {
      ...
      if(valid) {
        ...
        .then(() => {
          this.$notify({
            title: "Success",
            message: "receiptCompoundVouchers Entry Created",
            type: "success"
          });
          this.$router.push("/accounting/receipt-compound-vouchers");
        });
      } else {
        this.$notify({
          title: "Error",
