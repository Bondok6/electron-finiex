<template>
  <div class="mt-2 action-buttons-nonGrown justify-center align-baseline">
    <el-button size="mini" class="mb-1" type="info" @click="create()">{{
      $t("save-f5")
    }}</el-button>
...
export default {
  methods: {
    create() {
      this.$store.dispatch("Accounting/clientPaymentBond/create").then(() => {
        this.$notify({
          title: "Success",
          message: "client payment bond Created successfully",
          type: "success"
        });
