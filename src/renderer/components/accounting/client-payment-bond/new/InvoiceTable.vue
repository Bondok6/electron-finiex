  ...
  <el-container class="container ma-4 mt-0 mb-0 invoice-table new-record-table">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-select
            class="mobile-width-full selectButton"
  ...
  computed: {
    ...mapState({
      ...
      banksOrFundsSelected: (state) =>
        state.Accounting.clientPaymentBond.banksOrFundsSelected,
      banksList: (state) => state.lists.banksList,
      invoicesList: (state) => state.Accounting.clientPaymentBond.invoicesList,
      distributionStatus: (state) =>
        state.Accounting.clientPaymentBond.distributionStatus,
    }),
  },
  ...
  watch: {
    invoicesList: {
      handler: function (val, oldVal) {
        // if (val.length == 0) {
        //   // this.tableData = [{ ...this.appenedTableData }];
        // } else {
        this.tableData = val.map(
          ({ remainAmount, dateToPay, dateCreated, voucherNumber }) => {
            return {
              invoiceAmount: remainAmount, //مبلغ الفاتوره
              invoiceDate: dateCreated, //تاريخ الفاتوره
              dueDate: dateToPay, //تاريخ الاستحقاق
              invoiceNo: +voucherNumber, //رقم الفاتوره
              costCenterId: 0, //مركز التكلفه
              payTypeId: 0, //طريقه الدفع
              bankId: 0, //البنك
              checkNo: 0, //رقم السند
              ...
              voucherDescription: "", //وصف,
              voucherAmount: 0, //المبلغ المدفوع
              remainAmount: remainAmount, // المبلغ المتبقي بحسبه من عندي
              showDialog: false,
            };
          }
        );
