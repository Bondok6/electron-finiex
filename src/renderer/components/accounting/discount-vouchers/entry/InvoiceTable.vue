<template>
  <el-container class="container ma-4 mt-0 mb-0 invoice-table">
    <el-table :data="data" style="width: 100%" stripe border max-height="750">
      ...
      >
        <template slot-scope="scope">
          <NuxtLink
            :to="
              localePath(
                '/accounting/discount-vouchers/edit/' + scope.row.voucherID
              )
            "
          >
      ...
      />
      <el-table-column align="center" prop="billNumber" :label="$t('invoice-number')">
        <template slot-scope="scope">
          {{ scope.row.billNumber == ""  ? $t("without") : scope.row.billNumber }}
        </template></el-table-column
      >
      <el-table-column align="center" prop="total" :label="$t('total')" />
      <el-table-column align="center" :label="$t('bond-status')">
        <template slot-scope="scope">
