<template>
  <div class="mt-2 action-buttons-nonGrown justify-center align-baseline">
    <!-- /accounting/supplier-payment-bond -->
    ...
    <NuxtLink :to="localePath('/accounting/supplier-payment-bond')">
...
export default {
  ...
  computed: {
    reportData(){
      return {
        reportPath: reportsPaths["supplier-voucher"],
        headerPath: reportsPaths["headerCompany"],
        dataSet: `uri=${this.$config.axios.baseURL}accounting/supplier-vouchers/${this.$route.params.id};jpath=$;Header$Authorization=bearer${" "+localStorage.getItem("accessToken")}`,
        connString: `endpoint=${this.$config.axios.baseURL}accounting/supplier-vouchers/${this.$route.params.id};Header$Authorization=bearer${" "+localStorage.getItem("accessToken")}`,
      }
    }
  },
  methods: {
    deleteRecord() {
      this.$confirm(this.$t("message-when-delete-record"), "Warning", {
        ...
        .then(() => {
          return this.$store.dispatch("Accounting/supplierPaymentBond/delete", {
            id: this.$route.params.id
          });
        })
        .then(() => {
          this.$router.push("/accounting/supplier-payment-bond");
          this.$message({
            type: "success",
    ...
    update() {
      this.$store
        .dispatch("Accounting/supplierPaymentBond/update")
        .then(() => {
          this.$notify({
            title: "Success",
            message: "supplier Payment Bond Update Successfully",
            type: "success"
          });
          this.$router.push("/accounting/supplier-payment-bond");
        })
