<template>
  ...
  <el-button size="mini" class="mb-1 btn-blue" @click="update()">{{
    ...
    <NuxtLink :to="localePath('/accounting/creditor-notice/allowed-discount')">
...
export default {
  ...
  computed: {
    reportData(){
      return {
        reportPath: reportsPaths["allowed-discount-edit"],
        headerPath: reportsPaths["headerCompany"],
        dataSet: `uri=${this.$config.axios.baseURL}accounting/credit-vouchers/${this.$route.params.id};jpath=$;Header$Authorization=bearer${" "+localStorage.getItem("accessToken")}`,
        connString: `endpoint=${this.$config.axios.baseURL}accounting/credit-vouchers/${this.$route.params.id};Header$Authorization=bearer${" "+localStorage.getItem("accessToken")}`,
      }
    }
  },
  methods: {
    update() {
      this.$store
        .dispatch("Accounting/creditorNotice/allowedDiscount/update")
        .then(() => {
          this.$notify({
            title: "Success",
          ...
          this.$router.push("/accounting/creditor-notice/allowed-discount");
        });
    },
    deleteRecord() {
      this.$confirm(this.$t("message-when-delete-record"), "Warning", {
        ...
        .then(() => {
          return this.$store.dispatch(
            "Accounting/creditorNotice/allowedDiscount/delete",
            {
