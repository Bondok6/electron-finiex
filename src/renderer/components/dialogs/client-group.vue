              ...
              <table style="width: 100%">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="input-padding">
                      <el-select v-model="form.customerType" class="width-full">
                        <el-option
                          :label="$t('activated')"
                          :value="1"
                        ></el-option>
...
export default {
  ...
  methods: {
    ...mapMutations({
      updateDialogState: "customerManagement/customerGroup/updateDialogState",
      setEditMode: "customerManagement/customerGroup/setEditMode"
    }),
    closeDialog() {
    ...
    save() {
      if (this.editMode) {
        //  Edit Mode
             this.$store
          .dispatch("customerManagement/customerGroup/update", this.form)
          .then(() => {
            this.closeDialog();
            this.$message.success("Group Updated Successfully");
            this.$store.dispatch("customerManagement/customerGroup/fetchRecords" , this.searchParams);
          });
      } else {
        // Create Mode
        this.$store
          .dispatch("customerManagement/customerGroup/create", this.form)
          .then(() => {
            this.closeDialog();
    ...
    deleteRecord() {
      this.$store
        .dispatch("customerManagement/customerGroup/delete",{id : this.form.id})
        .then(() => {
          this.closeDialog();
  ...
  computed: {
    ...
    ...mapState({
      editMode: state => state.customerManagement.customerGroup.editMode,
      dialogState: state => state.customerManagement.customerGroup.dialogState,
      searchParams : state => state.customerManagement.customerGroup.searchParams
    }),
    showDialog: {
      set(state) {
