...
export const actions = {
  async fetchRecords(
    ...
    try {
      let API = 'inventory/storesupplyorder/list'
  ...
  fetchMaxCode() {
    return this.$axios.get(`inventory/storesupplyorder/newinvoiceid`);
  },
  ...
  create({ state }) {
    ...
    return this.$axios.post("inventory/storesupplyorder/create", data);
  },
  async editSingleRecordDetails({ commit }, searchParams = {}) {
    this.commit("loaded", false, { root: true });
    try {
      const res = await this.$axios.get(`inventory/storesupplyorder/get`, { params: searchParams });
  ...
  update({ state }) {
    let data = JSON.stringify(state.recordDetails);
    console.log(state.recordDetails)
    return this.$axios.put("inventory/storesupplyorder/update", data);
  },
  delete(_, code) {
    return this.$axios.delete(`inventory/storesupplyorder/${code}`);
  },
}