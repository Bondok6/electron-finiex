...
export const actions = {
  suggestCode() {
    return this.$axios("general/units/max-code");
  },
  ...
  create(_, payload) {
    ...
    return this.$axios.post("general/units", record);
  },
  update(_, payload) {
    let record = new FormData();
    for (let key in payload) {
    ...
    return this.$axios.put("general/units", record);
  },
  delete(_, { id }) {
    return this.$axios.delete(`general/units/${id}`);
  },
  fetchSingleRecord(_, { id }) {
    return this.$axios.get(`general/units/${id}`);
  }
};
