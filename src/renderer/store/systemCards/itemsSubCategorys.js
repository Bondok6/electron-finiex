...
export const actions = {
  suggestCode() {
    return this.$axios("general/sub-items-categories/max-code");
  },
  async fetchItemsCategorys(
    { commit },
  ...
  create(_, payload) {
    ...
    return this.$axios.post("general/sub-items-categories", record);
  },
  update(_, payload) {
    let record = new FormData();
    for (let key in payload) {
    ...
    return this.$axios.put("general/sub-items-categories", record);
  },
  delete(_, { id }) {
    return this.$axios.delete(`general/sub-items-categories/${id}`);
  },
  fetchSingleRecord(_, { id }) {
    return this.$axios.get(`general/sub-items-categories/${id}`);
  }
};
