...
export const actions = {
  suggestCode() {
    return this.$axios("general/items-attributes/max-code");
  },
  ...
  create(_, payload) {
    ...
    return this.$axios.post("general/items-attributes", record);
  },
  update(_, payload) {
    let record = new FormData();
    for (let key in payload) {
    ...
    return this.$axios.put("general/items-attributes", record);
  },
  delete(_, { id }) {
    return this.$axios.delete(`general/items-attributes/${id}`);
  },
  fetchSingleRecord(_, { id }) {
    return this.$axios.get(`general/items-attributes/${id}`);
  }
};
